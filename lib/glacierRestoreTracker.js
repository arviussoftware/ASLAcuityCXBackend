...
export async function createSourceS3Client() {
  ...
  const accessKeyId =
    ...
    decryptEnvKey(process.env.AWS_ACCESS_KEY_ID) ||
    decryptEnvKey(process.env.Amazon_ACCESS_KEY_ID) ||
    "";
  const secretAccessKey =
    creds.AWS_SECRET_ACCESS_KEY ||
    creds.Amazon_SECRET_ACCESS_KEY ||
    decryptEnvKey(process.env.AWS_SECRET_ACCESS_KEY) ||
    decryptEnvKey(process.env.Amazon_SECRET_ACCESS_KEY) ||
    "";
...
export async function writeRestoreRecord({
  ...
  try {
    ...
    if (dbStatus === "READY" || dbStatus === "NOTIFIED") {
      ...
      await executeStoredProcedure("usp_markglacierrestoreneedsretrieval", {
        interactionid: Number(interactionId),
        payloadtype: payloadType,
      });
    } else if (dbStatus === "STANDARD") {
      await executeStoredProcedure("usp_markglacierrestorestandard", {
        interactionid: Number(interactionId),
        payloadtype: payloadType,
      });
    }
