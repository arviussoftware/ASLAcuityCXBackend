...
export async function initiateObjectRestore({
  ...
  if (current.status === "retrieved") {
    await writeRestoreRecord({ interactionId, filePath, status: "retrieved", createdBy });
    return current;
  }
...
export async function upsertGlacierRestoreRequest({
  ...
  // usp_UpsertGlacierRestoreRequest returns a TABLE, not outputmsg/statuscode,
  // so no third argument is needed here.
  const result = await executeStoredProcedure(
