...
export async function POST(request) {
  try {
    ...
    const parameters = {
      ...
      querytype: queryType,
      timezone: timezone,
      userId: loggedInUserId, // ✅ NEW LINE HERE
    };
    ...
    const data = await executeStoredProcedure(
      "usp_GetAverageEvaluationScoreByOrganization",
      parameters
    );
    ...
    if (queryType === 0) {
      ...
      const OrgAvgEvaluationData = data.recordsets[0];
      ...
      return NextResponse.json({
        ...
        data: {
          OrgAvgEvaluationData: OrgAvgEvaluationData,
