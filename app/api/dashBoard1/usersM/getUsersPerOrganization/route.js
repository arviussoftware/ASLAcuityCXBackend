...
export async function GET(request){
    try{
        ...
        const result = await getUsersPerOrganization(loggedInUserId);
        if(result?.recordsets?.length > 0){
            const orgData = result.recordsets[0];
...
async function getUsersPerOrganization(currentUserId) {
    const inputParams = {
      currentUserId,
    };
