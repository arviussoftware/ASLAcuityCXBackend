...
export async function GET(request) {
  try {
    ...
    const dashboardSummary = await getFormDashboardSummary(loggedInUserId);
...
// Helper function to execute the stored procedure and get the form dashboard summary
async function getFormDashboardSummary(currentUserId) {
  const inputParams = {
    currentUserId, // Pass the current user ID to the stored procedure
  };
  try {
    // Execute the stored procedure to get the form dashboard summary
    const result = await executeStoredProcedure(
      "usp_FormDashboardSummary", // Stored procedure name
      inputParams, // Input parameters
      outputmsgWithStatusCodeParams // Output message and status code parameters
    );
    return result; // Return the result from the stored procedure
  } catch (error) {
