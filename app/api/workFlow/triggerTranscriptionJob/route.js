...
function resolveSourceType(filePath, dbSourceType) {
  ...
  const t = (dbSourceType || "").toLowerCase();
...
async function markFailed(interactionId, errorCode, errorMessage) {
  ...
  // Try with transcription_status and transcription_error params first
  try {
    ...
    // SP may not have transcription_status/error param — retry with legacy format
    try {
      await executeStoredProcedure(
        "usp_UpdateInteractionTranscription",
        {
      ...
      // Final fallback if transcription_error is also missing
      try {
        await executeStoredProcedure(
          "usp_UpdateInteractionTranscription",
          {
        ...
        console.warn(
          `[STT] Failed to write error status to DB for ${interactionId}:`,
          dbErr.message,
        );
      }
    }
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          `[STT] Initial processing mark failed for ${interactionId}:`,
          dbErr.message,
        );
      }
    }
    ...
    const accessStatusCode = Number(accessCheck?.output?.statuscode ?? 500);
    ...
    if (accessStatusCode !== 200) {
      const errMsg =
        accessCheck?.output?.outputmsg ||
        "Failed to verify transcription access.";
    ...
    const cfgResult = await executeStoredProcedure(
    ...
    logError("POST /api/workFlow/triggerTranscriptionJob", err);
    const { code, message: friendlyMsg, retryable } = classifyError(err);

    console.error(
      `[STT] ${code} — interaction ${body?.interactionId}: ${err.message}`,
    );
...
async function fetchAudioBuffer(filePath, sourceType, creds) {
  ...
  if (isS3 || sourceType === "aws-s3") {
    ...
    const s3Client = new S3Client({
      region: creds.REGION,
      credentials: {
        accessKeyId: creds.Amazon_ACCESS_KEY_ID,
        secretAccessKey: creds.Amazon_SECRET_ACCESS_KEY,
      },
    });
...
async function callOpenAIWhisper(
  ...
  try {
    ...
    console.error(`[STT] Diarization failed: ${diarErr.message}`);
  }
